...
## ⚡ Migration notice: Unipile removed → official Meta WhatsApp Cloud API + referral handoff added

Everything below this box is the original phase-by-phase history and is kept for context, but
**the Unipile-based WhatsApp integration it describes (`unipileClient.js`, `chat_id` tracking,
`UNIPILE_*` env vars) has been removed.** WhatsApp now goes through the **official Meta WhatsApp
Cloud API** (Graph API) instead — no third-party relay, direct to Meta.

**What changed:**
- `src/services/whatsapp/unipileClient.js` is gone. New
  `src/services/whatsapp/metaWhatsappClient.js` sends messages via
  `POST /{api_version}/{phone_number_id}/messages` on `graph.facebook.com` — one function for
  approved-template sends (`sendTemplateMessage`), one for plain-text sends within the 24h
  customer-service window (`sendTextMessage`).
- `POST /api/whatsapp/webhook` now parses Meta's `entry[].changes[].value.messages[]` payload
  shape (not Unipile's `chat_id`/`is_sender` shape). A new `GET /api/whatsapp/webhook` route
  handles Meta's one-time webhook-verification handshake (`hub.challenge`).
- **The first business-initiated message is sent as normal text only when Meta accepts that the customer-service window is open; otherwise the backend automatically falls back to an approved WhatsApp message
  TEMPLATE** — this is a Meta platform rule (business-initiated messages outside
  the 24h window must be templates). Set `META_WHATSAPP_OPENING_TEMPLATE_NAME` /
  `META_WHATSAPP_OPENING_TEMPLATE_LANGUAGE` (or the equivalent fields on the Settings page) once
  your template is approved in WhatsApp Manager. Only once the lead actually **replies** to that
  template does the real AI conversation (property discovery, everything in `promptBuilder.js`)
  start — same as before, unchanged.
- `Conversation.unipileChatId` is gone (Meta has no per-thread chat-id concept — every send just
  targets the lead's phone number). Replaced with `Conversation.templateSent`/`templateSentAt`.
- **New referral/handoff closing step**: once a property conversation looks wrapped up and the
  lead seems satisfied, the AI asks once whether they'd like to be connected to a referral contact
  (default name "Monica", configurable) for the next step. If the lead says yes, the app appends
  the real configured phone number to the reply itself (never generated by the AI, to avoid any
  chance of a wrong/hallucinated number) and won't ask again for that lead. Tracked on
  `Conversation.referralStatus` (`none` → `asked` → `accepted`/`declined`); configured via
  `Settings.referral.{enabled,personName,contactNumber}` (falls back to `REFERRAL_*` env vars).
- `GET /api/whatsapp/status` now reports the Meta-connected number (`display_phone_number`,
  `verified_name`, `quality_rating`) instead of Unipile's account info.
- **Google Calendar booking** (from the previous migration) is unchanged.

**Required `.env` additions** — see `.env.example` in this project for the full list with
explanations:
```
...
META_WHATSAPP_APP_SECRET=...           # optional but recommended
META_WHATSAPP_OPENING_TEMPLATE_NAME=...
...
1. Create/open a Meta App at developers.facebook.com → add the **WhatsApp** product.
2. Under WhatsApp → API Setup, note the **Phone number ID** and **WhatsApp Business Account ID**.
3. Create a **System User** (Business Settings → Users → System Users) with
   `whatsapp_business_messaging` + `whatsapp_business_management` permissions, generate a
   **permanent token** for it — use this as `META_WHATSAPP_ACCESS_TOKEN` (the temporary token shown
   on the API Setup test page expires in 24h, don't use that in production).
4. Under WhatsApp → Configuration → Webhook, set the Callback URL to
   `https://your-domain.com/api/whatsapp/webhook` and the Verify Token to whatever you set as
   `META_WHATSAPP_VERIFY_TOKEN` → click Verify and Save → subscribe to the `messages` field.
5. Under WhatsApp Manager → Message Templates, create and submit your opening template for
   approval (e.g. a short "Hi {{1}}, this is X from Y, reaching out about your property enquiry"
   template). Once **Approved**, set its exact name/language as
   `META_WHATSAPP_OPENING_TEMPLATE_NAME` / `META_WHATSAPP_OPENING_TEMPLATE_LANGUAGE`.
